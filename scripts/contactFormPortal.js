...
window.addEventListener("authChecked", async function () {
  ...
  function renderSubmission(sub) {
    ...
    summary.innerHTML = `
			...
			<span class="contactform__summary-message">${(sub.message || "-").slice(
        0,
        50
      )}${sub.message && sub.message.length > 50 ? "..." : ""}</span>
      ...
      deleteBtn.addEventListener("click", function () {
        confirmModal(
          ...
          function (result) {
            if (!result) return;
            if (loadingBar) loadingBar.style.display = "block";
            fetch(URL_BASE + `/api/admin/contact-form/submission/${sub.id}`, {
              method: "DELETE",
              headers: {
                Authorization: `Bearer ${
                  getTokenFromSession ? getTokenFromSession() : ""
              ...
              .then(async (response) => {
                ...
                if (!response.ok) {
                  ...
                  showAlert(
                    json.message || text || "Error deleting submission.",
                    true,
                    alertDiv
                  );
                  return;
