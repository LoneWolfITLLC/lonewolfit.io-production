...
window.addEventListener("authChecked", async function () {
    const loadingBar = document.getElementById("contactFormSubmissionsLoadingBar");
    ...
    const refreshBtn = document.getElementById("refreshContactFormSubmissionsBtn");
    if (!listDiv) return;
    ...
    async function fetchSubmissions() {
        ...
        listDiv.innerHTML = "";
        try {
            const response = await fetch(URL_BASE + "/api/admin/contact-form/submissions", {
                headers: { Authorization: `Bearer ${getTokenFromSession ? getTokenFromSession() : ''}` }
            });
            if (!response.ok) {
            ...
            if (!submissions.length) {
                listDiv.innerHTML = '<div class="main__text main__item-centered">No contact form submissions found.</div>';
                return;
            }
            submissions.forEach(sub => renderSubmission(sub));
        } catch (err) {
            showAlert("Network error fetching submissions: " + err.message, true, alertDiv);
        } finally {
    ...
    function renderSubmission(sub) {
        ...
        details.innerHTML = `
            <div class="contactform__user">
                <span class="contactform__user-label">Name:</span> <span class="contactform__user-name">${sub.name || "-"}</span>
                <span class="contactform__user-label">Email:</span> <a href="mailto:${sub.email || ''}" class="contactform__user-email">${sub.email || "-"}</a>
                <span class="contactform__user-label">Phone:</span> <a href="tel:${sub.phone || ''}" class="contactform__user-phone">${sub.phone || "-"}</a>
            </div>
            <div class="contactform__message"><strong>Message:</strong><br>${sub.message || "-"}</div>
            <div class="contactform__actions">
                <button class="btn btn-primary contactform__call-btn" title="Call Client" ${sub.phone ? "" : "disabled"}>Call Client</button>
                <button class="btn btn-secondary contactform__email-btn" title="Send Email to Client" ${sub.email ? "" : "disabled"}>Send Email</button>
        ...
        if (deleteBtn) {
            deleteBtn.addEventListener("click", async function () {
                ...
                if (loadingBar) loadingBar.style.display = "block";
                try {
                    const response = await fetch(URL_BASE + `/api/admin/contact-form/submission/${sub.id}`, {
                        method: "DELETE",
                        headers: { Authorization: `Bearer ${getTokenFromSession ? getTokenFromSession() : ''}` }
                    });
                    const text = await response.text();
