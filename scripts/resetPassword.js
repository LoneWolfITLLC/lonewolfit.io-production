      ...
      try {
        ...
        if (response.ok) {
          ...
          if (text.includes("Google Account")) {
            confirmModal(
              "It seems your email is associated with a Google Account. Would you like to sign in with Google instead?",
              function (confirmGoogleSignIn) {
                if (confirmGoogleSignIn) {
        ...
        if (response.ok) {
          ...
          if (response.status === 404) {
            const errmodal = alertModal(
              "Reset password session not found or expired. Please try again after waiting for the redirection...",
              true
            );
            setTimeout(() => {
