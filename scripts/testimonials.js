    ...
    try {
      ...
      if (!response.ok) {
		...
		if (json.message && json.message.trim() === "Malformed token") {
			...
			updateNavButtons();
			setTimeout(() => {
			  window.location.href = "login.html?redirect_uri=testimonials.html";
			}, 3000);
			return;
		}
        alertModal("Error Fetching Unapproved Testimonials: " + (json.message || text || "Unknown error"));
        if (loadingBar) loadingBar.style.display = "none";
        updateNavButtons();
        return;
      }
      ...
      alertModal("Error fetching testimonials: " + error.message);
      updateNavButtons();
    }
  } else {
    alertModal("Fetching Unapproved Testimonials failed.");
    updateNavButtons();
  }
  if (loadingBar) loadingBar.style.display = "none";
}
...
async function deleteTestimonial(id, context = "unapproved") {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const json = JSON.parse(text);
	  if(json.message && json.message.trim() === "Malformed token") {
		showMessage("Token expired. Please login again...", false, context);
		setTimeout(() => {
		  window.location.href = "login.html?redirect_uri=testimonials.html";
		}, 3000);
		if (loadingBar) loadingBar.style.display = "none";
      ...
      showMessage("Error Deleting Testimonial: " + (json.message || text || "Unknown error"), false, context);
      if (loadingBar) loadingBar.style.display = "none";
      busy = false;
      return;
    }
...
window.addEventListener("authChecked", async function () {
    ...
    ?.addEventListener("click", async function () {
      ...
      try {
        ...
        if (!response.ok) {
		  ...
		  const json = JSON.parse(text);
		  if(json.message && json.message.trim() === "Malformed token") {
			showMessage("Token is expired. Please login again...", false, "unapproved");
			setTimeout(() => {
			  window.location.href = "login.html?redirect_uri=testimonials.html";
			}, 3000);
			if (loadingBar) loadingBar.style.display = "none";
          ...
          showMessage(
            "Error Approving Testimonial: " + (json.message || text || "Unknown error"),
            false,
            "unapproved"
          );
          if (loadingBar) loadingBar.style.display = "none";
  ...
  // Add click-to-refresh on unapproved testimonials slideshow wrapper, excluding nav buttons
  const unapprovedWrapper = document.querySelector(
    ".unapprovedTestimonials__wrapper, .slideshow__wrapper"
  );
  if (unapprovedWrapper) {
    let isRefreshing = false;
    unapprovedWrapper.addEventListener("click", async function (e) {
      const navBtn = e.target.closest(
        "#prev, #next, .slideshow__nav-btn--prev, .slideshow__nav-btn--next"
      );
    ...
    ?.addEventListener("click", async function () {
      ...
      try {
        ...
        if (!response.ok) {
          const text = await response.text();
          const json = JSON.parse(text);
          console.error("Error response:", json || text);
          if (json.message && json.message.trim() === "Malformed token") {
            showMessage(
            ...
            setTimeout(() => {
              window.location.href =
                "login.html?redirect_uri=testimonials.html";
            }, 3000);
            if (loadingBar) loadingBar.style.display = "none";
            busy = false;
            return;
          ...
          showMessage(
            "Error: Could not save the testimonial: " + (json.message || text),
            false,
            currentEditContext
          );
          if (loadingBar) loadingBar.style.display = "none";
