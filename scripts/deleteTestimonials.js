...
async function fetchApprovedTestimonials() {
	...
	try {
		...
		if (!response.ok) {
			...
			throw new Error(
				"Failed to fetch approved testimonials: " + (json.message || text)
			);
		}
		...
		alertModal("Error fetching approved testimonials. Please try again later.");
	} finally {
	...
	confirmModal(
		...
		async function (confirmDelete) {
			...
			try {
				...
				displaySlides();
				alertModal("Testimonial deleted successfully!");
			} catch (error) {
